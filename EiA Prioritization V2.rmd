...
CombPlot_historic<-reactive({ terra::mask(
      terra::crop(x = droplevels(HazComb()[["MeanHaz"]][["historic-historic"]][["RastReclass"]]), Geographies[Geographies$NAME_2 %in% input$Admin2,]),
      Geographies[Geographies$NAME_2 %in% input$Admin2,])
  })

CombPlot_future<-reactive({ terra::mask(
      terra::crop(x = droplevels(HazComb()[["MeanHaz"]][[input$Future]][["RastReclass"]]), Geographies[Geographies$NAME_2 %in% input$Admin2,]),
      Geographies[Geographies$NAME_2 %in% input$Admin2,])
  })
...
CombPlotProp_historic<-reactive({ terra::mask(
      terra::crop(x = HazComb()[["PropHaz"]][["historic-historic"]][["RastReclass"]], Geographies[Geographies$NAME_2 %in% input$Admin2,]),
      Geographies[Geographies$NAME_2 %in% input$Admin2,]
      )})

CombPlotProp_future<-reactive({ terra::mask(
      terra::crop(x = HazComb()[["PropHaz"]][[input$Future]][["RastReclass"]], Geographies[Geographies$NAME_2 %in% input$Admin2,]),
      Geographies[Geographies$NAME_2 %in% input$Admin2,]
      )})
...
DT_Data2_historic<-reactive({
  PrepTable(Data=HazXRegion(),
                   ...
                   A1=input$Admin1,
                   A2=input$Admin2,
                   Table="SPAM")[,VoP:=round(VoP,1)][,Scenario:="historic"]
})
...
DT_Data2_future<-reactive({
  PrepTable(Data=HazXRegion(),
                   ...
                   A1=input$Admin1,
                   A2=input$Admin2,
                   Table="SPAM")[,VoP:=round(VoP,1)][,Scenario:=input$Future]
})
...
DT2_diff_fun<-function(historic,future){
  ...
  if("Admin2" %in% cols_out){
    merged<-melt(merged,id.vars=c("Admin1","Admin2","Crop","Hazard"),variable.name = "Scenario",value.name = "VoP")
  }else{
    merged<-melt(merged,id.vars=c("Admin1","Crop","Hazard"),variable.name = "Scenario",value.name = "VoP")
  }
  
  merged[,Crop:=gsub(" _vop","",Crop)]
...
DT_Data2<-reactive({
  DT2_diff_fun(historic=DT_Data2_historic(),future=DT_Data2_future())
})
...
renderDT({
  datatable(if(input$MakeWide=="Yes"){
    if(AdminLevel()=="Admin2"){
      dcast(DT_Data2()[VoP>input$MinVoP,VoP:=round(VoP,1)],Admin1+Admin2+Scenario+Crop~Hazard,value.var="VoP")
    }else{
      dcast(DT_Data2()[VoP>input$MinVoP,VoP:=round(VoP,1)],Admin1+Scenario+Crop~Hazard,value.var="VoP") 
    }
    }else{
